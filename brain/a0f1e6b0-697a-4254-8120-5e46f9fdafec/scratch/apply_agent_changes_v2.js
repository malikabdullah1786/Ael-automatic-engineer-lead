...
function findFunctionRange(lines, startKeyword, endKeyword) {
  let startIndex = -1;
  let endIndex = -1;
...
// 1. Add meetingTime to AgentState
...
// Find the closing brace of overdueTasksQueue
...
// Re-read joined to prevent off-by-index errors
...
const routeIntentStart = lines.findIndex(l => l.startsWith('async function routeIntentNode('));
const routeIntentEnd = lines.findIndex(l => l.startsWith('async function standupNode('));
  ...
  if (classified.intent === "list_meetings") {
    ...
    if (tickets) {
      for (const t of tickets) {
        if (t.error_context && t.error_context.startsWith("[Scheduled:")) {
          const match = t.error_context.match(/^\\\\[Scheduled:\\\\s*([^\\\\s|]+)\\\\s*\\\\|\\\\s*Link:\\\\s*([^\\\\]]+)\\\\]\\\\s*(.*)$/);
          if (match) {
    ...
    if (filtered.length === 0) {
      ...
      for (const m of filtered) {
        content += \`| **\${m.devName}** | \${m.projectName} | \\\`\${m.time.toLocaleString()}\\\` | [Google Meet](\${m.meetLink}) | \\\`\${m.status}\\\` | \${m.reason} |\\n\`;
      }
    }
...
const checkWorkloadStart = lines.findIndex(l => l.startsWith('async function checkWorkloadNode('));
const checkWorkloadEnd = lines.findIndex(l => l.startsWith('async function prepActionNode('));
...
const resolveIdentityStart = lines.findIndex(l => l.startsWith('async function resolveIdentityNode('));
const resolveIdentityEnd = lines.findIndex(l => l.startsWith('async function prepActionNode('));
...
const prepActionStart = lines.findIndex(l => l.startsWith('async function prepActionNode('));
const prepActionEnd = lines.findIndex(l => l.startsWith('async function executeActionNode('));
...
const executeActionStart = lines.findIndex(l => l.startsWith('async function executeActionNode('));
...
const workloadRouteStart = lines.findIndex(l => l.startsWith('function determineWorkloadRoute('));
const workloadRouteEnd = lines.findIndex(l => l.startsWith('function determineApprovalRoute('));
...
const identityRouteStart = lines.findIndex(l => l.startsWith('function determineIdentityRoute('));
const identityRouteEnd = lines.findIndex(l => l.startsWith('function determineActionRoute('));
